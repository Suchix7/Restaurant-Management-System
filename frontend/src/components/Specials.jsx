...
const Specials = () => {
  ...
  const fetchEvents = async () => {
    try {
      const res = await axiosInstance.get("/specials");
      setEvents(res.data || []);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axiosInstance.delete(`/specials/${id}`);
  ...
  const handleUpdate = async () => {
    try {
      ...
      await axiosInstance.put(`/specials/${editingEvent._id}`, data, {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-slate-900 mb-2">Our Specials</h2>
      <p className="text-slate-600 mb-6">
        Create or update upcoming special events.
      </p>

      {events.length === 0 ? (
        <p className="text-slate-500 italic">No special events scheduled.</p>
      ) : (
